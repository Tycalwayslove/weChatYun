...
/*<jdists trigger="prod">
const STATIC_SERVER_URL = 'https://6465-demo-yun-1259313865.tcb.qcloud.la'
</jdists>*/

const BACKGROUND_PERFIXER = `${STATIC_SERVER_URL}/bg`;
const WEATHER_IMAGE_PERFIXER = `${STATIC_SERVER_URL}/icon`;

const KEY = "tyccc"; //和风天气中应用的key名称
const USER_ID = "HE1905291011541174"; //和风天气中应用的密钥ID
...
const $ = {
  ...
  getWeatherImage(code, isNight) {
    return (
      WEATHER_IMAGE_PERFIXER +
      "/" +
      path.join(isNight ? "night" : "day", `${code}.svg`)
    );
  },
  getBackgroundImage(weather, isNight) {
    return (
      BACKGROUND_PERFIXER +
      "/" +
      path.join(isNight ? "night" : "day", `${weather}.jpg`)
    );
  },
  timeFormat(pattern = "YYYY-MM-DD", d) {
    ...
    for (let i in o) {
      pattern = pattern.replace(new RegExp("(" + i + "+)", "gi"), function(
        a,
        b
      ) {
